 ...
 * @author José
 */
@WebFilter(filterName = "sessionFilterSuperAdministrador", urlPatterns = {"/superadmin/*"})
public class SessionFilterSuperAdministrador implements Filter {

    private static final boolean debug = true;
    private FilterConfig filterConfig = null;

    public SessionFilterSuperAdministrador() {
    }
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        if (session.sessionStatus() != null && session.sessionStatus()) {
            if (session.getSession() != null && session.getPerfil() != null
                    && session.getPerfil().compareTo(ConstanteBeacon.COD_PERFIL_SUPERADMIN) == 0) {
                try {
